    ...
    <main>
      ...
      <div class="grid">
        <div class="stack">
          ...
          <section>
            <h2>Custom domain</h2>
            <p>Add <code>frontend/CNAME</code> with one hostname, such as <code>app.example.com</code>. After deploy, create a proxied DNS <code>CNAME</code> for that host pointing to <code>w7s.cloud</code>.</p>
          </section>
        </div>

        <section>
          <h2>GitHub Action</h2>
          <p>Add this workflow as <code>.github/workflows/deploy.yml</code>. W7S verifies deploy access with the repo's GitHub token.</p>
          <pre><code>${escapeHtml(actionYaml)}</code></pre>
        </section>
      </div>
    </main>
  </body>
