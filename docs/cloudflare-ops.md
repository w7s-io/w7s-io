...
The internal `checkUsageLimit(...)` helper can report `wouldBlock: true` when projected usage would exceed policy. This is for upcoming expensive primitives and is not wired to block existing traffic.
