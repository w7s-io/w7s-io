...
The internal `checkUsageLimit(...)` helper reports `wouldBlock: true` when projected usage would exceed policy. Deploy, RPC, queue-send, and workflow-start requests are wired to this check.

These counters are approximate because KV updates are read-modify-write operations. Treat them as operational/product visibility and abuse protection, not strict billing-grade quota accounting.
